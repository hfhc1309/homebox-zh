<script setup lang="ts">
  ...
  async function deleteEntry(id: string) {
    const result = await confirm.open("你确定要删除这一个条目吗？");
...
<template>
  <div v-if="log">
    <BaseModal v-model="entry.modal">
      <template #title>
        {{ entry.id ? "Edit Entry" : "新条目" }}
      </template>
      <form @submit.prevent="dispatchFormSubmit">
        <FormTextField v-model="entry.name" autofocus label="名称" />
        <DatePicker v-model="entry.completedDate" label="完成时间" />
        <DatePicker v-model="entry.scheduledDate" label="预计时间" />
        <FormTextArea v-model="entry.description" label="笔记" />
        <FormTextField v-model="entry.cost" autofocus label="花费" />
